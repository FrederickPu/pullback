...
lemma SSADo.continuationTypesAlign_push
    ...
    SSADo.continuationTypesAlign (vars.push (var, ty)) kMutVars kbreak kcontinue contTy := sorry
...
lemma SSADo.continuationValuesAlign_push
    ...
    (hkb : kbreak ≠ some var) (hkc : kcontinue ≠ some var) :
    SSADo.continuationValuesAlign (args.push (var, v)) kbreak kcontinue onBreak onContinue := sorry
...
lemma SSADo.compilesTo_expr_false
    ...
    compileProp vars mutVars kMutVars kbreak kcontinue args inloop onBreak onContinue
        (SSADo.expr e) (cast (by grind) c) := sorry
...
lemma SSADo.compilesTo_expr_true
    ...
    (hunit : (e.eval args).map (·.inferType) = some (.ofBase .unit)) :
    compileProp vars mutVars kMutVars kbreak kcontinue args inloop onBreak onContinue
        (SSADo.expr e) (cast (by grind) (DoResult.pure (LoopStep.continue st))) := sorry
...
lemma SSADo.compilesTo_seq
    ...
    compileProp vars mutVars kMutVars kbreak kcontinue args inloop onBreak onContinue (SSADo.seq s₁ s₂) x := sorry
...
lemma SSADo.compilesTo_letE
    ...
    compileProp vars mutVars kMutVars kbreak kcontinue args inloop onBreak onContinue
        (SSADo.letE var val rest) x := sorry
...
lemma SSADo.compilesTo_letM
    ...
    compileProp vars mutVars kMutVars kbreak kcontinue args inloop onBreak onContinue
        (SSADo.letM var val rest) x := sorry
...
lemma SSADo.compilesTo_assign
    ...
    compileProp vars mutVars kMutVars kbreak kcontinue args inloop onBreak onContinue
        (SSADo.assign var val rest) x := sorry
...
lemma SSADo.compilesTo_loop_return
    ...
    compileProp vars mutVars kMutVars kbreak kcontinue args inloop onBreak onContinue
        ...
        (if h : inloop then cast (by grind) (DoResult.return x) else cast (by grind) x) := sorry
...
lemma SSADo.compilesTo_break
    ...
    (hContVal : SSADo.continuationValuesAlign args kbreak kcontinue onBreak onContinue) :
    compileProp vars mutVars kMutVars kbreak kcontinue args inloop onBreak onContinue
        SSADo.break (cast (by grind) (DoResult.pure (LoopStep.break st))) := sorry
...
lemma SSADo.compilesTo_continue
    ...
    (hContVal : SSADo.continuationValuesAlign args kbreak kcontinue onBreak onContinue) :
    compileProp vars mutVars kMutVars kbreak kcontinue args inloop onBreak onContinue
        SSADo.continue (cast (by grind) (DoResult.pure (LoopStep.continue st))) := sorry
...
lemma SSADo.compilesTo_return
    ...
    compileProp vars mutVars kMutVars kbreak kcontinue args inloop onBreak onContinue
        (SSADo.return out)
        (if h : inloop then cast (by grind) (DoResult.return res) else cast (by grind) res) := sorry
...
lemma SSADo.compilesTo_ifthenelse_return
    ...
    compileProp vars mutVars kMutVars kbreak kcontinue args inloop onBreak onContinue
        (SSADo.ifthenelse c t e rest) (cast (by grind) (DoResult.return x)) := sorry
...
lemma SSADo.compilesTo_ifthenelse_rest
    ...
    (ih : compileProp vars mutVars kMutVars kbreak kcontinue args inloop onBreak onContinue rest x) :
    compileProp vars mutVars kMutVars kbreak kcontinue args inloop onBreak onContinue
        (SSADo.ifthenelse c t e rest) x := sorry
...
| expr e => do
    ...
    if h : !inloop then
        pure ⟨cast (by grind) c, compilesTo_expr_false h hc⟩
    else
...
| letE var val rest => do
    ...
    let r ← rest.evalprop (vars.push (var, v.inferType)) mutVars kMutVars kbreak kcontinue
        ...
        (continuationTypesAlign_push hContTy sorry sorry)
        (continuationValuesAlign_push hContVal sorry sorry)
    pure ⟨r.val, compilesTo_letE hv r.property⟩
| letM var val rest => do
    if args.any (·.1 == var) then none
    let before ← get
    let ⟨v, hv⟩ ← (val.eval args).attach
    ...
    let r ← rest.evalprop (vars.push (var, v.inferType)) (mutVars.push (var, v.inferType)) kMutVars
        ...
        (continuationValuesAlign_push hContVal sorry sorry)
    set before
    pure ⟨r.val, compilesTo_letM hv r.property⟩
...
| ifthenelse c t e rest =>
    if h_cond : c.eval args != SSAConst.ofInt (0 : Int) then do
        ...
        if h : inloop then
            ...
            | .pure _ =>
                (rest.evalprop vars mutVars kMutVars kbreak kcontinue args inloop onBreak onContinue contTy
                    hArgs hContTy hContVal).map fun r => ⟨r.val, compilesTo_ifthenelse_rest r.property⟩
        else
            have ih : compileProp vars mutVars kMutVars kbreak kcontinue args inloop onBreak onContinue
                    (if c.eval args != SSAConst.ofInt 0 then t else e) r.val := by
                simp [h_cond]; exact r.property
