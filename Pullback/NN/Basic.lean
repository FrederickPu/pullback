...
def NDMatrix.map {α : Type u} {β : Type v} (f : α → β) : {shape : List Nat} → NDMatrix α shape → NDMatrix β shape
| [] => f
...
def NDMatrix.broadcastMapAux {α : Type u} {β : Type v} {s₁ s₂ : List Nat} (f : NDMatrix α s₁ → NDMatrix β s₂) : (shapePrefix : List Nat) → NDMatrix α (shapePrefix ++ s₁) → NDMatrix β (shapePrefix ++ s₂)
| [] => f
| _::l => fun x => fun i => NDMatrix.broadcastMapAux f l (x i)

def NDMatrix.broadcastMap {α : Type u} {β : Type v} {s₁ s₂ : List Nat} (f : NDMatrix α s₁ → NDMatrix β s₂) {shape : List Nat} (hShape : shape.drop (shape.length - s₁.length) = s₁ := by simp) : NDMatrix α shape → NDMatrix β (shape.take (shape.length - s₁.length) ++ s₂) :=
  cast (by {
...
def NDMatrix.broadcastMap? {α : Type u} {β : Type v} {s₁ s₂ : List Nat} (f : NDMatrix α s₁ → NDMatrix β s₂) : {shape : List Nat} → NDMatrix α shape → Option (NDMatrix β (shape.take (shape.length - s₁.length) ++ s₂)) :=
  fun {shape} x =>
...
def NDMatrix.broadcastMap! {α : Type u} {β : Type v} [Inhabited β] {s₁ s₂ : List Nat} (f : NDMatrix α s₁ → NDMatrix β s₂) : {shape : List Nat} → NDMatrix α shape → NDMatrix β (shape.take (shape.length - s₁.length) ++ s₂) :=
  fun {shape} =>
    if h : shape.drop (shape.length - s₁.length) = s₁ then
